...
export async function launchPreviewInWebView({
  ...
  html = html.replace("ws://127.0.0.1:23625", translateExternalURL(`ws://127.0.0.1:${dataPlanePort}`));
  ...
  await vscode.env.asExternalUri(vscode.Uri.parse(translateExternalURL(`http://127.0.0.1:${dataPlanePort}`)));
  return panel;
}
